...
LL_SCORERS_HTML = "ll_scorers.html"
LL_YELLOW_HTML = "ll_yellow.html"
LL_RED_HTML = "ll_red.html"
LL_ASSISTS_HTML = "ll_assists.html"
OUT_PATH = "index.html"
...
# ---------- Extract embedded stats data from laliga.com's own stat pages ----------
def load_laliga_stats(html_path):
    html = open(html_path).read()
    m = re.search(r'<script id="__NEXT_DATA__" type="application/json">(.*?)</script>', html, re.S)
    data = json.loads(m.group(1))
    return data["props"]["pageProps"]["statsData"]["player_rankings"]

ll_scorers = load_laliga_stats(LL_SCORERS_HTML)
ll_yellow = load_laliga_stats(LL_YELLOW_HTML)
ll_red = load_laliga_stats(LL_RED_HTML)
ll_assists = load_laliga_stats(LL_ASSISTS_HTML)

def ll_stat(player, name):
...
def _norm_team_name(name):
    ...
    name = re.sub(r"\b(FC|CF|SAD|SA|AFC|Club de Futbol|Futbol Club)\b", "", name, flags=re.I)
    return re.sub(r"[^a-z0-9]", "", name.lower())

TLA_ALIAS = {"ATM": "ATL", "RAC": "SAN"}

crest_by_tla = {t["team"]["tla"]: t["team"]["crest"] for t in table}
crest_by_name = {_norm_team_name(t["team"]["name"]): t["team"]["crest"] for t in table}

def ll_team_crest(player):
    tla = player["team"]["shortname"]
    tla = TLA_ALIAS.get(tla, tla)
    if tla in crest_by_tla:
        return crest_by_tla[tla]
    return crest_by_name.get(_norm_team_name(player["team"]["name"]), "")
...
combined_assists_note = "" if any_real_assists else """<div class="note">⚠ This combined table's assist numbers aren't fully populated by its data source yet — see "Most Assists" below for real, complete figures pulled directly from LaLiga's own site.</div>"""

# ---------- Single-stat lists from LaLiga's own site (real data) ----------
def ll_stat_table_head(label):
    return f"""<thead><tr><th class="team">Player</th><th>Club</th><th>Mins Played</th><th>{label}</th></tr></thead>"""

def ll_stat_row(player, stat_key):
    value = ll_stat(player, stat_key)
    if value is None:
        return ""
    mins = ll_stat(player, "total_mins_played") or 0
    crest = ll_team_crest(player)
    crest_html = f'<img src="{crest}" alt="" class="crest">' if crest else ""
    return f"""
    <tr>
      <td class="team">{crest_html}{player['name']}</td>
      <td>{player['team']['shortname']}</td>
      <td>{int(mins)}</td>
      <td class="pts">{int(value)}</td>
    </tr>"""

ASSISTS_TABLE_HEAD = ll_stat_table_head("Assists")
CARDS_TABLE_HEAD_Y = ll_stat_table_head("Yellow Cards")
CARDS_TABLE_HEAD_R = ll_stat_table_head("Red Cards")

ll_assists_rows = "".join(ll_stat_row(p, "total_assists") for p in ll_assists) or "<tr><td colspan=4>No assist data yet</td></tr>"
ll_yellow_rows = "".join(ll_stat_row(p, "total_yellow_card") for p in ll_yellow) or "<tr><td colspan=4>No card data yet</td></tr>"
ll_red_rows = "".join(ll_stat_row(p, "total_red_card") for p in ll_red) or "<tr><td colspan=4>No card data yet</td></tr>"
...
<title>LaLiga 2026-27 Tracker</title>
<style>
  :root {{
    ...
    --accent: #ee8a09; --tab-bg: #eee6d6; --tab-active: #ee8a09; --tab-active-text: #ffffff;
  }}
  @media (prefers-color-scheme: dark) {{
    :root:not([data-theme="light"]) {{
      ...
      --accent: #ffab40; --tab-bg: #2a2534; --tab-active: #ffab40; --tab-active-text: #16131a;
    }}
  }}
...
<div class="wrap">
  ...
  <div id="tab-table" class="tab-panel active">
    <div class="card">
      ...
      <div class="note">European qualification spots can shift based on cup-competition winners already qualifying via league position — actual allocations confirm toward season's end. Relegation sends the bottom 3 down to Segunda División.</div>
    </div>
    ...
    <div class="card">
      ...
      <div class="note">"Safety" modeled as ~{SAFETY_THRESHOLD} points, a rough historical LaLiga survival benchmark.</div>
    </div>
  </div>

  <div id="tab-club" class="tab-panel">
    <div class="explainer">
      <b>New to LaLiga?</b> The table tells you <i>where</i> a team stands, but not <i>how</i> they got there. These stats show the underlying strengths and weaknesses — a team can have a good record while quietly being fragile defensively, or vice versa.
    </div>

    <div class="card">
      <h2>Defensive Strength: Clean Sheets</h2>
      <div class="explainer">A <b>clean sheet</b> is a game where a team doesn't concede at all. It's the single clearest sign of defensive solidity — LaLiga title winners are almost always among the league's clean-sheet leaders, because you can't lose a game you don't concede in.</div>
      <table>
    ...
    <div class="card">
      ...
      <div class="explainer">Some teams are much stronger at home than away (or the reverse) — this is one of the oldest storylines in football, and it's especially pronounced in Spain given the size and atmosphere of some LaLiga stadiums. <b>Points per game (PPG)</b> at home vs. away shows exactly how lopsided that split is. <b>Form</b> is the last 5 results (most recent last) — a better read on momentum than the season-long record.</div>
      <table>
    ...
    <div class="card">
      ...
      <div class="explainer">Goal margin matters beyond the 3 points — a big win boosts goal difference (which breaks ties in the table) and can be a statement result against a rival, especially in a Clásico or Derbi.</div>
  ...
  <div id="tab-player" class="tab-panel">
    <div class="explainer">
      <b>New to LaLiga?</b> The <b>Pichichi Trophy</b> (top scorer) is the league's most prestigious individual award. But goals alone don't capture everything a player contributes — this table adds context.
    </div>
    <div class="card">
      <h2>Pichichi Race &amp; Goal Involvements</h2>
      <div class="explainer">
        <b>Goals</b>: the headline number, and what decides the Pichichi Trophy.<br>
    ...
    <div class="explainer" style="margin-top: 4px;">
      <b>Want just one ranking at a time?</b> The sections below break out individual rankings — Assists, Yellow Cards, and Red Cards pull real numbers directly from LaLiga's own site.
    </div>

    <details class="stat-accordion">
      <summary>Most Goals <span class="sub">The Pichichi race — decided by goals alone, nothing else</span></summary>
      <div class="accordion-body">
        <div class="explainer"><b>Goals</b> is the headline number and what the actual Pichichi Trophy is decided by. It's the most-watched individual stat in the league, but it rewards finishers over creators — see "Most Goals & Assists" below for the fuller picture.</div>
        <table>{PLAYER_TABLE_HEAD}<tbody>{goals_rows or "<tr><td colspan=8>No scorer data yet</td></tr>"}</tbody></table>
      </div>
    </details>
    ...
    <details class="stat-accordion">
      ...
      <div class="accordion-body">
        ...
        <table>{ASSISTS_TABLE_HEAD}<tbody>{ll_assists_rows}</tbody></table>
      </div>
    </details>
    ...
    <details class="stat-accordion">
      ...
      <div class="accordion-body">
        <div class="explainer"><b>Goal Involvements</b> (goals + assists) gives a fuller picture of a player's attacking output than the Pichichi table does on its own. A player with 8 goals and 10 assists has been directly involved in 18 goals — arguably more valuable to their team than someone with 12 goals and 0 assists, even though the latter would top the pure scoring chart.</div>
    ...
    <details class="stat-accordion">
      ...
      <div class="accordion-body">
        ...
        <table>{CARDS_TABLE_HEAD_Y}<tbody>{ll_yellow_rows}</tbody></table>
      </div>
    </details>
    ...
    <details class="stat-accordion">
      ...
      <div class="accordion-body">
        ...
        <table>{CARDS_TABLE_HEAD_R}<tbody>{ll_red_rows}</tbody></table>
      </div>
    </details>
  </div>

  <footer>Data: football-data.org &amp; laliga.com · Rebuilt periodically, not live-updating</footer>
</div>
<script>
